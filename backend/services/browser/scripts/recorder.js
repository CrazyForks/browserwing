...
if (window.__browserwingRecorder__) {
	...
	var createRecorderUI = function() {
		...
		panel.style.cssText = 'position:fixed;top:20px;right:20px;z-index:999999;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Helvetica,Arial,sans-serif;width:320px;background:white;border-radius:8px;box-shadow:0 4px 20px rgba(0,0,0,0.15);border:1px solid #e5e7eb;overflow:hidden;';	// 创建头部区域（可拖动）
		var header = document.createElement('div');
		header.style.cssText = 'padding:14px 16px;background:#fafafa;cursor:move;user-select:none;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e5e7eb;';		var headerLeft = document.createElement('div');
		...
		buttonArea.style.cssText = 'padding:12px 16px;display:flex;gap:8px;border-bottom:1px solid #e5e7eb;background:white;';
	...
	stopRecordingBtn.onclick = async function() {
		...
		for (var i = 0; i < menuItems.length; i++) {
			...
			item.style.cssText = 'padding:8px 12px;cursor:pointer;font-size:13px;border-radius:4px;color:#374151;transition:background 0.15s;';
	...
	var addActionToList = function(action, index) {
		...
		var item = document.createElement('div');
		item.style.cssText = 'padding:10px 12px;margin:4px 0;background:white;border:1px solid #e5e7eb;border-radius:6px;font-size:12px;transition:all 0.2s;';
		...
		typeLabel.style.cssText = 'font-weight:600;color:#111827;';
		...
		indexLabel.style.cssText = 'font-size:11px;color:#9ca3af;font-weight:500;';
		...
		details.style.cssText = 'color:#6b7280;font-size:11px;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
